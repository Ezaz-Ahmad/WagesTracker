 ...
 * invalid unless that row still exists, is unrevoked, unexpired, and
 * belongs to the same user the JWT claims to be.
 *
 ...
 * (a stolen database dump would hand over live, directly usable
 * credentials instead of an opaque id).
 */

/** Matches the JWT's own expiry (`TOKEN_TTL` in ../auth.ts) — the session
 * row and the token it backs are meant to expire together. */
export const SESSION_TTL_MS = 30 * 24 * 60 * 60 * 1000;

/** Throttle window for `last_seen_at` writes — see touchSessionIfStale. */
...
/** Creates a new session row for a freshly issued token (signup, login, or
 * the replacement token after a password change) and returns its id, ready
 * to embed as the JWT's `sid` claim. */
export async function createSession({ userId, userAgent, ipAddress }: CreateSessionInput): Promise<string> {
  const id = randomUUID();
  ...
  await db.execute({
    sql: `INSERT INTO user_sessions (id, user_id, user_agent, ip_address, created_at, last_seen_at, expires_at)
          VALUES (?, ?, ?, ?, ?, ?, ?)`,
    args: [id, userId, userAgent, ipAddress, nowIso, nowIso, expiresAt],
  });
  return id;
}
...
export async function validateSession(
  ...
  if (row.expires_at <= new Date().toISOString()) return { valid: false };
...
/** Only the caller's own currently-valid (unrevoked, unexpired) sessions —
 * used for the Settings "Security & Sessions" list. */
