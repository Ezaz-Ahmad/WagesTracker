...
## Shift-in-progress notification (iOS)

> **Status:** built on `feature/ios-shift-notification`. Not part of the current TestFlight build. **Currently paused** — despite the delivery-visibility fix below, the notification still wasn't reliably showing up on a physical device, with no failure reported either. Rather than ship a Settings toggle and background-posting path for a feature that isn't reliably working, it's disabled at a single kill-switch (`isShiftNotificationFeatureEnabled()` in `platform/shiftNotifications.ts`, defaulting to `false`) while it's revisited — the Settings toggle renders nothing and `useTodayShift.start()` never posts, but every other line of the implementation described below is untouched and the whole feature description remains accurate for when it resumes. See `docs/shift-notification-feature-pause.md`.

Native iOS only, same app-local-plugin pattern as biometric login above: a small purpose-built native bridge (`ios/App/App/ShiftNotificationPlugin.swift`), no new npm dependency. While a shift is open, a local notification titled "Shift in progress" sits in the notification center with a "Sign out" action button, so ending a shift doesn't require reopening the app at all.

**This is not Android's "foreground service" notification, and can't be.** iOS has no concept of a truly undismissable notification — the user can always swipe this one away manually, at any time, and doing so does **not** end the shift; the shift just keeps running with nothing visible reminding the user about it, exactly as if the notification had never been posted. What this feature guarantees is narrower and still useful: the notification appears the moment a shift starts, survives backgrounding, locking, and the app being closed outright, and only goes away on its own once the shift actually ends (in-app, or via its own "Sign out" button) — not on a timer, not on an OS whim.

**How "Sign out" from the notification works without the app running.** Tapping the action can briefly relaunch/resume the app process in the background to deliver the response, but that execution window is short and unreliable for spinning up a full WebView/JS bridge from cold — so the PATCH to end the shift is made directly in native Swift (`URLSession`), not routed through Capacitor/JS. This is why `UNUserNotificationCenter`'s delegate and the notification category are registered in `AppDelegate.application(_:didFinishLaunchingWithOptions:)` rather than in `MainViewController` (see that file's comments): a background-only launch may never reach `capacitorDidLoad()` at all in that particular process lifetime, and the delegate has to be ready regardless.

**Offline / interrupted "Sign out" — retried automatically, never silently lost.** If the native background request can't complete (no connectivity, or the OS reclaims the execution window before the request finishes), the shift is deliberately left running rather than guessed at, and the intended sign-out time is recorded locally (`UserDefaults`, no token in it) as a pending record. The next time the app is actually opened, `AppContext` finishes that PATCH automatically through the same API client the in-app Sign out button uses, before showing anything else — the user never has to notice or manually redo it. The pending record is cleared after exactly one attempt either way (success or failure), so a persistently-failing reconciliation can never turn into a retry loop on every launch; the ordinary in-app Sign out button is always still there as a fallback if something about that one shift is permanently wrong.

**Credential storage.** Starting a shift writes a dedicated Keychain item (`ios/App/App/ShiftNotificationPlugin.swift`'s `ShiftNotificationCenter`) holding just what the background PATCH needs to authenticate itself: the shift id, the API origin, and the current session's bearer token — the same trust boundary as the token already stored behind biometrics above, not a new one, and not gated on biometrics itself (there is no user present to authenticate a background wake). It's deleted the instant the notification's job is done, either because the shift ended through the app (the notification is cleared) or because the background "Sign out" attempt ran (successfully or not) — never left behind stale.

**Turning it off.** `Settings → Security → Shift notification` (`ShiftNotificationSettings.tsx`) lets a user turn this off entirely on a per-device basis — added after the feature itself shipped, for anyone who'd rather not have a persistent notification for every shift. It's a plain device-local preference (`isShiftNotificationEnabled`/`setShiftNotificationEnabled` in `platform/shiftNotifications.ts`, backed by `localStorage`, never synced to the backend — the same pattern as the remembered-email convenience in `lib/api.ts`), defaulting to **on** so existing installs keep the notification they already had. `useTodayShift`'s `start()` checks it before ever calling `postShiftStartedNotification`, so turning it off skips the call outright rather than relying on the adapter to swallow it. Deliberately scoped to *future* shifts only — it does not reach into `shifts`/app state to retroactively post or clear a notification for one already in progress (a shift that's already running keeps whatever notification it already has until it ends normally); this keeps the toggle a simple, self-contained preference rather than coupling a Settings screen to shift state it otherwise has no reason to know about. Native iOS only, same `Capacitor.isNativePlatform()` gating as `BiometricLoginSettings` — the underlying feature doesn't exist on web/PWA at all, so there is nothing for the control to toggle there.

**Making a platform failure visible.** `postShiftStartedNotification` never throws (see above), which used to mean a real failure — permission denied, a Keychain write error, or anything else `ShiftNotificationPlugin.swift` can reject with — was reported to nobody but the device console via `console.error`. Found on physical-device testing: notification permission was granted and a fresh build was installed, yet nothing ever appeared, with no way for the person using the app to tell the difference between "working, just nothing to show yet" and "silently broken." `postShiftStarted` now resolves to a `ShiftNotificationResult` (`{ ok: true } | { ok: false, error: string }`) instead of `void` — still never throwing, still never delaying or blocking the shift-start button — and `useTodayShift.start()` inspects it once it settles. An `ok: false` result is reported through a dedicated `shiftNotificationNotice` banner on `AppContext` (warning tone, dismissible, same `StatusBanner` component as every other in-app notice) rather than reusing `actionError`'s danger tone — the shift itself already started successfully; only the reminder didn't show up.

**Testing.** No macOS/Xcode toolchain is available in this sandbox, so the actual notification delivery, background wake, and Keychain behavior cannot be compiled or exercised here. Covered instead: the JS-side translation layer (`platform/__tests__/nativeShiftNotifications.test.ts`, mirroring the biometric adapter's own tests — including that a native failure now resolves to `{ ok: false, error }` rather than only logging), the shared web/native contract's default behavior (`platform/__tests__/shiftNotifications.test.ts`), `useTodayShift`'s start/end wiring (`lib/__tests__/useTodayShift.notifications.test.tsx` — notification posted with the right payload on start, cleared only after a successful sign-out PATCH, never blocks the button on a native failure, skipped entirely when the Settings toggle below has turned the feature off, and reports a `shiftNotificationNotice` only on `ok: false`), the on/off preference itself (`platform/__tests__/shiftNotificationPreference.test.ts` against real `localStorage`, plus `lib/__tests__/useShiftNotificationSetting.test.tsx` and `settings/__tests__/ShiftNotificationSettings.test.tsx` for the hook/component that expose it in Settings), AppContext's pending-reconciliation effect (`context/__tests__/shiftNotificationReconciliation.test.tsx` — runs once per signed-in session, finishes the PATCH, clears the pending record on both success and ordinary failure, and defers to the normal expired-session logout on a 401), and the failure banner end to end through the real `<App/>` (`context/__tests__/shiftNotificationFailureNotice.test.tsx` — appears in warning tone (not danger) on `ok: false`, never appears on `ok: true`, and is dismissable). `frontend/scripts/verify-ios-plugin-registration.mjs` additionally checks, without compiling Swift, that `ShiftNotificationPlugin.swift` is actually wired into the Xcode target and that AppDelegate sets up the notification delegate/category. Like biometric login, this still needs GitHub's macOS Simulator workflow plus physical-device verification before shipping — background notification-action delivery in particular is not something a simulator run alone can fully confirm.
