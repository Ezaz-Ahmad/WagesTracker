...
- **Continuous roster adaptation** — four consecutive shifts at new hours replace an older routine, two conflicting shifts pause reminders while the change is uncertain, and a three-week weekday gap starts a fresh learning era. Abandoned weekdays expire automatically while newly worked weekdays become ready after four reliable occurrences.
...
- **Signup email validation without ownership confirmation** — signup validates the address shape on both the client and server, blocks malformed values, and pauses on likely provider-domain typos (for example `@hmail.com`) so the user can accept the suggested correction or explicitly keep what they entered. The app never silently rewrites the address and does not send a confirmation link for a new account; a successful signup creates a login-ready session immediately. Verification remains required when an authenticated user changes their existing login email under Settings → Security.
