...
The project has 158 automated tests: 111 backend integration/API tests (`backend/test/`), written with [Vitest](https://vitest.dev/) and [Supertest](https://github.com/ladjs/supertest) and exercising the real Express app end-to-end over HTTP, each test file getting its own isolated, throwaway temporary SQLite database (see `backend/test/testApp.ts`) so runs never share or pollute data; plus 47 frontend tests (`frontend/src/lib/__tests__/` and `frontend/src/screens/__tests__/`), also Vitest, covering wage/duration calculations, week aggregation, PDF report data, password-policy validation, and the session-management API client, including that the session list is refreshed against the replacement token right after a password change. Of the 111 backend tests, 30 specifically cover authentication and password security — `passwordPolicy.test.ts`, `change-password.test.ts`, `passwordHashing.test.ts`, and `passwordMigration.test.ts` — described in [Authentication and password security](#authentication-and-password-security) below, 18 cover database-backed sessions — `sessions.test.ts` and `sessions-management.test.ts` — described in [Sessions](#sessions) below, and 5 (`authRateLimiter.test.ts`) prove that the signup/login rate limiter never blocks an authenticated logout request. Almost all frontend tests are pure-logic tests run in a plain Node environment (no DOM) for speed, but `screens/__tests__/SettingsScreen.test.tsx` (2 tests) is a real component test — it renders `<SettingsScreen />` with [jsdom](https://github.com/jsdom/jsdom) and [@testing-library/react](https://testing-library.com/docs/react-testing-library/intro/), drives it through actual user input via [@testing-library/user-event](https://testing-library.com/docs/user-event/intro/), and proves the password-change session-list refresh end to end — it fails if that behavior is ever removed from `SettingsScreen.tsx`, which a pure API-level test can't do since it never touches the component's own code. That one file opts into jsdom itself via a `// @vitest-environment jsdom` pragma rather than switching the whole suite over to it (see `vitest.config.ts`).
