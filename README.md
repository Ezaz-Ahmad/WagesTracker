...
- **Frontend** — the "Security & Sessions" section in Settings lists sessions with the current one labeled "This device," lets you log out any other individual session or all of them at once (both require confirmation via the app's existing confirm-dialog pattern and refresh the list afterward), and shows loading/error/success states around all of it. It never renders a raw JWT.
