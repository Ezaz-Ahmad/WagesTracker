...
**How it relates to Remember Me.** The two are independent, deliberately non-conflicting persistence paths, not the same setting under two names:
- **Remember Me** (the existing checkbox) keeps a token in `localStorage`, restored silently on every app launch with no further gate.
- **Biometric login** stores its own, separate token copy behind a Keychain item that requires a live Face ID/Touch ID match to read — see below. If Remember Me is already on, the ordinary silent restore wins and biometrics is never even invoked (nothing to unlock, already unlocked). If Remember Me is off, biometric login is what lets a cold launch skip retyping a password: without it, a session-only login normally has to be repeated after every app-process restart.
- Turning biometric login on does not change whether Remember Me is checked, and vice versa.

**Enabling it** (`Settings → Security`) requires being signed in already, and prompts Face ID/Touch ID immediately — the setting only turns on if that prompt succeeds; cancelling or failing it leaves the setting off with no error banner (an expected, ordinary outcome, not a failure worth explaining). A successful enable stores the current session token behind a fresh Keychain item and overwrites any credential that was already there, so there is never more than one stored credential and never a stale one left over from a previous account on a shared device.
...
**Cleanup.** Both Keychain items are cleared, unconditionally and without prompting, on: logout, password change (the backend already revokes every session on a password change — see above — so the credential would be dead anyway; clearing it means re-enabling is a conscious choice rather than a silent carry-over), revoking the current session from Settings, and account deletion. "Log out all other devices" does not touch *this* device's own credential (its own session was never revoked), but a device revoked *that* way clears itself the next time it tries to use its now-dead credential and gets a 401 from the mandatory backend validation above — there's no live cross-device channel to reach into another device's Keychain directly, by design.
