...
function formatDiagnostics(d: ViewportDiagnostics): string {
  const lines = [
    ...
    `vv.scale          ${n(d.visualViewportScale)}`,
...
export function ViewportDebugOverlay() {
  ...
  const rows: [string, string][] = [
    ...
    ["vv.scale", n(diagnostics.visualViewportScale)],
    ...
    [
      "guard",
      diagnostics.recoveryActive
        ? `${diagnostics.recoveryMode} (${diagnostics.recoverySource}, ${n(diagnostics.recoveryHeldMs)}ms)`
        : "off",
    ],
